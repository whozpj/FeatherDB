...
public class Table {
    ...
    public void addRowWithID(List<String> row) throws InsertInsufficientValuesException, InsertTooManyValuesException {
        ...
        dataValues.insert(new ArrayList<>(row));
    }
    ...
    public void addColumn(String name) throws ColumnAlreadyExistException, UsingReservedWordException {
        ...
        dataValues.updateRows(row -> true, row -> row.add("NULL"));
    }
    ...
    public void dropColumn(String name) throws NoColumnFoundException, IDColumnNotUpdatableException {
        ...
        dataValues.updateRows(row -> true, row -> row.remove(colIdx));
    }
    ...
    public String[][] getDataValues() {
        List<List<String>> allRows = dataValues.getAllRows();
        int rowAmount = allRows.size();
        int columnAmount = columnNames.size();
        String[][] values = new String[rowAmount][columnAmount];
        for (int i = 0; i < rowAmount; i++) {
            for (int j = 0; j < columnAmount; j++) {
                values[i][j] = allRows.get(i).get(j);
            }
        }
    ...
    public List<List<String>> getDataValuesByColumns(List<String> cols) throws NoColumnFoundException {
        ...
        for (int i = 0; i < rowAmount; i++) {
            ...
            for (int j = 0; j < columnAmount; j++) {
                newDataValues.get(i).add(allRows.get(i).get(selectedIndexes[j]));
            }
        }
        return newDataValues;
    }

    public void updateDataValues(List<List<String>> newDataValues) throws NoDataValueProvidedException {
        if (newDataValues == null) {
            throw new NoDataValueProvidedException();
        }
        this.dataValues = new BTree();
        for (List<String> row : newDataValues) {
            this.dataValues.insert(new ArrayList<>(row));
        }
    }
